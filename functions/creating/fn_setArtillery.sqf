...
If(_do_it)then
{
  ...
  while{missionNamespace getVariable [STRVAR_DO(artillery_ctrl),false]}do
  {
    ...
    {
       _sounds = [1,2,3,2,1,2,2,1,2,2,3,3,2,1,2,3,2,1,1,2,2,3,1,2,3,1,3,2,2,2,1,1,2,2,2,3,3,2,2,1,2,2,1,2,2,1,2,2];
       _sound_shuffle = _sounds call BFUNC(arrayShuffle);
       _pl_sound = format["incoming_%1",(selectRandom _sound_shuffle)];
