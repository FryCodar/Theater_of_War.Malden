...
params ["_vec_name"];
  ...
  If(count (missionNamespace getVariable [STRVAR_DO(resp_store),[]]) > 0)then
  {
    ...
    If(_found)then
    {
      _work_part = _work_arr select _idx; ARR_MINIDX(_work_arr,_idx);
      missionNamespace setVariable [STRVAR_DO(resp_store),_work_arr,false];
      hint str (missionNamespace getVariable STRVAR_DO(resp_store));
      _vec_type = _work_part select 1;_vec_pos = _work_part select 2;_vec_dir = _work_part select 3;
      ...
      _new_vec addEventHandler ["Killed",{REMOTE_TRIEXESM(_this,system,setRespawnVecs,([0,2] select isDedicated));}];
      If((getText(configfile >> "CfgVehicles" >> _vec_type >> "vehicleClass")) isEqualTo "Autonomous")then
      {createVehicleCrew _new_vec;};
      _work_part set [0,_new_vec]; _work_arr = missionNamespace getVariable STRVAR_DO(resp_store); ARR_ADDVAR(_work_arr,_work_part);
      missionNamespace setVariable [STRVAR_DO(resp_store),_work_arr,false];
      hint str (missionNamespace getVariable STRVAR_DO(resp_store));
    };
  };
